...
### Resolved — pinned python-codex32 GUI verifies identity before Core mutation

The earlier reviewed GUI called the mutating Core wallet fill/import before
showing the master fingerprint comparison. Bails now pins
`1938b604182b5553f4e724fd2aea814496a24cd4`, revalidated on 2026-09-25. That
revision derives and checks the recovered master fingerprint before descriptor
import and repeats the restore-only check immediately before unlocking or
creating a destination wallet. When a wallet record is available, a mismatch
aborts before Bitcoin Core is changed. Without a record, the GUI shows the
recovered fingerprint and requires an explicit warning acknowledgement before
restore. Fresh wallet creation does not treat its newly generated fingerprint
as pre-existing authentication; it asks the operator to record it instead.

### Accepted limitation — 32-bit BIP32 fingerprint is not strong active authentication

The fingerprint is useful for human identification but has only 32 bits. An
attacker who can substitute enough recovery shares can in principle grind a
different seed with the same fingerprint. The maintainer rejected a separate
256-bit recovery-commitment UX for this flow; the current gate therefore does
not claim to authenticate against deliberate threshold-share substitution.
Future encrypted wallet/descriptor backups can provide a stronger independent
wallet-identity check without adding another recovery secret for the user.
