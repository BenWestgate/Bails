...
- Install Sparrow as the temporarily supported wallet coordinator.
- Provide passphrase memorization assistance.

The repository still contains a legacy Codex32 wallet implementation, but that path is not supported on current stable Tails and is being removed. Replacement Codex32 restoration is tracked separately. The current Clone and Backup menu workflows are unfinished and are not supported capabilities.
