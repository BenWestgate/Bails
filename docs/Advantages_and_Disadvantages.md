...
CipherStick is a script that installs Bitcoin Core on Tails and supports Sparrow as its wallet coordinator. Wallet creation and private-key handling are delegated to the supported wallet software rather than implemented by CipherStick itself.
...
1. **External Wallet Coordination**: Sparrow is the supported coordinator, keeping CipherStick focused on installation and node setup instead of custom private-key handling.
